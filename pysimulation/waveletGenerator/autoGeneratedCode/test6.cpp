...
void Test6::initialisationWavelet (){
	createMatrix(2, 2) ;
	p_matWavelet = {0.5, 0.25, 0.25, 0.25, 0.0, 0.25, 0.25, 0.25, 0.0} ;
	p_matPatch = {0, 0, 0, 1, 1, 0, 1, 1, 1, -1, 0, -1, -1, -1, -1, 0, -1, 1} ;
	p_pace = 1 ;
}

///Greedy wavelet cleaning with neighbours surrounding signal
void Test6::greedyCleaningWithNeighbours (std::vector<float> & keepSignalRectOut, const std::vector<float> & signalRectIn, size_t nbRow, size_t nbCol) {
	//getting the mask
	std::vector<float> res = std::vector<float>(nbRow*nbCol, 0.0f) ;
	for (size_t i (1lu) ; i < nbRow -1 ; i++) {
		for (size_t j (1lu) ; j < nbCol -1 ; j++) {
			float c = 0.0f ;
			c += (0.5* signalRectIn[(i)*nbCol + (j)]) ;
			c += (0.25* signalRectIn[(i)*nbCol + (j+1)]) ;
			c += (0.25* signalRectIn[(i+1)*nbCol + (j)]) ;
			c += (0.25* signalRectIn[(i+1)*nbCol + (j+1)]) ;
			c += (0.25* signalRectIn[(i)*nbCol + (j-1)]) ;
			c += (0.25* signalRectIn[(i-1)*nbCol + (j-1)]) ;
			c += (0.25* signalRectIn[(i-1)*nbCol + (j)]) ;
...
void Test6::greedyCleaning (std::vector<float> & keepSignalRectOut, const std::vector<float> & signalRectIn, size_t nbRow, size_t nbCol) {
	//getting the mask
	std::vector<float> res = std::vector<float>(nbRow*nbCol, 0.0f) ;
	for (size_t i (1lu) ; i < nbRow -1 ; i++) {
		for (size_t j (1lu) ; j < nbCol -1 ; j++) {
