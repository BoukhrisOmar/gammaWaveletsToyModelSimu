...
void Walsh::initialisationWavelet (){
	createMatrix(2, 2) ;
	p_matWavelet = {0.5, 0.5, 0.5, 0.5} ;
	p_matPatch = {0, 0, 0, 1, 1, 0, 1, 1} ;
	p_pace = 2 ;
}

///Greedy wavelet cleaning with neighbours surrounding signal
void Walsh::greedyCleaningWithNeighbours (std::vector<float> & keepSignalRectOut, const std::vector<float> & signalRectIn, size_t nbRow, size_t nbCol) {
	//getting the mask
	...
	for (size_t i (1lu) ; i < nbRow -1 ; i++) {
		for (size_t j (1lu) ; j < nbCol -1 ; j++) {
			...
			c += (0.5* signalRectIn[(i)*nbCol + (j+1)]) ;
			c += (0.5* signalRectIn[(i+1)*nbCol + (j)]) ;
			c += (0.5* signalRectIn[(i+1)*nbCol + (j+1)]) ;
...
void Walsh::greedyCleaning (std::vector<float> & keepSignalRectOut, const std::vector<float> & signalRectIn, size_t nbRow, size_t nbCol) {
	//getting the mask
	std::vector<float> res = std::vector<float>(nbRow*nbCol, 0.0f) ;
	for (size_t i (1lu) ; i < nbRow -1 ; i++) {
		for (size_t j (1lu) ; j < nbCol -1 ; j++) {
